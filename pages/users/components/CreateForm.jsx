...
import { Button, Divider, Dropdown, Menu, message, Input, Tooltip, Modal, Form, DatePicker, TimePicker, Select, Cascader, InputNumber } from 'antd';
...
const CreateForm = ({ visible, onCreate, onCancel, loading }) => {
  ...
  return (
    <Modal
      ...
      footer={[
        <Button key='clean' loading={loading} onClick={() => {
          form.resetFields()
        }}>
          Clean
        </Button>,
    ...
    >
      ...
      >
        <Form.Item name='userName' label='Username' rules={[{
          ...
          <Form.Item name='dob' label='DOB' rules={[{
              required: true,
